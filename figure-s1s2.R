...
data$Group <- factor(data$Group, levels = c(GN, IN, ON, NN))
...
ggplot(melted_data, aes(x = PRA, y = Rate, color = Metric, group = Metric, linetype = Metric, shape = Metric)) +
  ...
  facet_wrap(~ Group, scales = "fixed", nrow = 2, ncol = 2) +
